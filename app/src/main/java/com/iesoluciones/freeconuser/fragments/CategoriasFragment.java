...
public class CategoriasFragment extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        CategoriasAdapter adapter = new CategoriasAdapter(new CategoriasAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(Categoria categoria) {
                //Mandar a otro fragment de recycler de servicios
                getFragmentManager().beginTransaction().replace(R.id.frameDrawer,ServiciosFragment.newInstance((int)categoria.getId()))
                        .addToBackStack("TAG")
                        .commit();
            }
        });
